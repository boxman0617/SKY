<?php
SkyL::Import(SkyDefines::Call('FACTORY_CLASS'));
spl_autoload_register('autoload_classes');
function autoload_classes($class_name)
{
    if(is_file(SkyDefines::Call('DIR_APP_MODELS').'/'.$class_name.'.model.php')) // Check for model
    {
        Event::PublishActionHook('/autoload_classes/before/model/', array($class_name));
        SkyL::Import(SkyDefines::Call('DIR_APP_MODELS').'/'.$class_name.'.model.php');
        Event::PublishActionHook('/autoload_classes/after/model/', array($class_name));
        return true;
    }
    if(is_file(SkyDefines::Call('DIR_APP_MAILERS').'/'.strtolower($class_name).'.mailer.php')) // Check for mailer
    {
        Event::PublishActionHook('/autoload_classes/before/mailer/', array($class_name));
        SkyL::Import(SkyDefines::Call('DIR_APP_MAILERS').'/'.strtolower($class_name).'.mailer.php');
