...
class DataCache
{
	...
	public static function GetCache($key)
	{
		$file = self::GetKeyFile($key);
		if($file !== false)
			return unserialize(file_get_contents(SkyDefines::Call('DIR_APP').self::$CacheDir.'/'.$file));
	...
	{
		file_put_contents(SkyDefines::Call('DIR_APP').self::$CacheDir.'/'.self::GetNewFileName($key), serialize($data));
	}

	protected static function ClearCache($keyFile)
	{
		@unlink(SkyDefines::Call('DIR_APP').self::$CacheDir.'/'.$keyFile);
	}
